...
{
  gattAttrType_t type; //!< Attribute type (2 or 16 octet UUIDs)	属性类型（2或16个八位字节UUID）
  uint8 permissions;   //!< Attribute permissions					属性权限
  uint16 handle;       //!< Attribute handle - assigned internally by attribute server	属性句柄 - 由属性服务器在内部分配
  uint8* const pValue; //!< Attribute value - encoding of the octet array is defined in 
                       //!< the applicable profile. The maximum length of an attribute 
                       //!< value shall be 512 octets.	属性值 - 数组的编码在适用的配置文件中定义。 属性值的最大长度应为512个八位字节。
} gattAttribute_t;
