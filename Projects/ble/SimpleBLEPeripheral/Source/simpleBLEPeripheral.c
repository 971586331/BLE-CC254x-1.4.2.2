...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  SimpleProfile_AddService( GATT_ALL_SERVICES );  // Simple GATT Profile	添加GATT服务
#if defined FEATURE_OAD
  ...
  // Register callback with SimpleGATTprofile	注册GATT服务的回调
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SBP_PERIODIC_EVT )
  {
    ...
    // Perform periodic application task	执行周期事件处理函数
    performPeriodicTask();
