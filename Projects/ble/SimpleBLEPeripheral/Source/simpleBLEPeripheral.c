...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  // Setup the GAP Peripheral Role Profile
  {
    ...
    // Set the GAP Role Parameters	ÉèÖÃGAP½ÇÉ«²ÎÊý
  ...
  // Set the GAP Characteristics	ÉèÖÃGAPÌØÕ÷
  GGS_SetParameter( GGS_DEVICE_NAME_ATT, GAP_DEVICE_NAME_LEN, attDeviceName );

  // Set advertising interval	ÉèÖÃ¹ã²¥¼ä¸ô
  {
  ...
  // Setup the GAP Bond Manager	ÉèÖÃGAP¹ÜÀí
  {
  ...
  // Initialize GATT attributes	//ÉèÖÃGATTÊôÐÔ
  ...
  SK_AddService( GATT_ALL_SERVICES ); // Simple Keys Profile	¼òµ¥ÃÜÔ¿ÅäÖÃÎÄ¼þ

  // Register for all key events - This app will handle all key events
  // ×¢²áËùÓÐ¹Ø¼üÊÂ¼þ - ´ËÓ¦ÓÃ³ÌÐò½«´¦ÀíËùÓÐ¹Ø¼üÊÂ¼þ
  ...
  // is halted
  // ÔÚÔÝÍ£Ê±ÆôÓÃÊ±ÖÓ·ÖÆµµ±ÎÞÏßµç´¦ÓÚ»î¶¯×´Ì¬ÇÒCC254x MCUÍ£Ö¹Ê±£¬Õâ»á½µµÍÓÐÐ§µçÁ÷
  HCI_EXT_ClkDivOnHaltCmd( HCI_EXT_ENABLE_CLK_DIVIDE_ON_HALT );

#if defined ( DC_DC_P0_7 )

  // Enable stack to toggle bypass control on TPS62730 (DC/DC converter)
  // ÆôÓÃ¶ÑÕ»ÒÔÔÚTPS62730£¨DC / DC×ª»»Æ÷£©ÉÏÇÐ»»ÅÔÂ·¿ØÖÆ
