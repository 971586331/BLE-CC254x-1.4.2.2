...
typedef enum
{
  GAPROLE_INIT = 0,                       //!< 等待开始 Waiting to be started
  GAPROLE_STARTED,                        //!< 开始但没有广告 Started but not advertising
  GAPROLE_ADVERTISING,                    //!< 正在广告 Currently Advertising
  GAPROLE_ADVERTISING_NONCONN,            //!< 正在使用没有连接的广告 Currently using non-connectable Advertising
  GAPROLE_WAITING,                        //!< 设备开始但测试广告，如果在等待期间前再次广告 Device is started but not advertising, is in waiting period before advertising again
  GAPROLE_WAITING_AFTER_TIMEOUT,          //!< 设备从一次连接但没有广告中超时，如果在等待期间前再次广告 Device just timed out from a connection but is not yet advertising, is in waiting period before advertising again
  GAPROLE_CONNECTED,                      //!< 进入一个连接 In a connection
  GAPROLE_CONNECTED_ADV,                  //!< 进入一个连接加广告 In a connection + advertising
  GAPROLE_ERROR                           //!< 发生错误，无效的状态 Error occurred - invalid state
} gaprole_States_t;
