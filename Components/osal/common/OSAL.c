...
void osal_run_system( void )
{
  ...
  do {
    if (tasksEvents[idx])  // Task is highest priority that is ready. ≈–∂œ∏ﬂ”≈œ»º∂µƒ»ŒŒÒ «∑Ò◊º±∏∫√
    {
      break;
    }
  } while (++idx < tasksCnt);
  ...
  if (idx < tasksCnt)
  {
    ...
    HAL_ENTER_CRITICAL_SECTION(intState);
    tasksEvents[idx] |= events;  // Add back unprocessed events to the current task. ±£¥ÊŒ¥±ª¥¶¿Ìµƒ ¬º˛
    HAL_EXIT_CRITICAL_SECTION(intState);
  }
